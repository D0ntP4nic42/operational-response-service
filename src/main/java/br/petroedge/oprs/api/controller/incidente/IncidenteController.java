...
public class IncidenteController {
    ...
    @Operation(
            ...
            description = "Endpoint para buscar incidentes com base em critérios de pesquisa.")
    public ResponseEntity<ApiResponse<PaginacaoResponse<BuscaIncidenteResponse>>> buscarIncidentes(
            @ParameterObject BuscaIncidenteRequest filtros,
            @ParameterObject @PageableDefault(size = 10, sort = "dtCriacao", direction = Sort.Direction.DESC, page = 0)
                    Pageable pageable) {

        var incidentes = incidenteService.buscarIncidentes(pageable, filtros);
        var response = new PaginacaoResponse<>(
                incidentes.getContent(),
                incidentes.getNumber(),
                incidentes.getSize(),
                incidentes.getTotalElements(),
                incidentes.getTotalPages());
    ...
    @Operation(
            ...
            description = "Endpoint para buscar um incidente específico pelo seu ID.")
    ...
    @PostMapping
    @Operation(
            summary = "Adicionar incidentes",
            description = "Endpoint para adicionar incidentes com base em um diagnóstico.")
    public ResponseEntity<ApiResponse<RecursoModificadoResponse<String>>> adicionarIncidentes(
            @RequestBody AdicionarIncidenteRequest request) {
    ...
    public ResponseEntity<ApiResponse<RecursoModificadoResponse<String>>> atualizarIncidente(
            String idIncidente, @RequestBody EditarIncidenteRequest request) {
